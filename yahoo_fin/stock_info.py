...
def get_dividends(ticker, start_date=None, end_date=None, index_as_date=True, headers=None):
...
def get_splits(ticker, start_date=None, end_date=None, index_as_date=True, headers=None):
...
### Earnings functions
def _parse_earnings_json(url, headers=None):
    if headers is None:
        headers = default_headers
...
def get_quote_data(ticker, headers=None):
    '''Inputs: @ticker
    ...
    if not resp.ok:
        raise AssertionError("""Invalid response from server.  Check if ticker is valid.""")
